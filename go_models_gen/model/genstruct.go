...
func (e *GenElement) Generate() string {
	...
	var p PrintAtom
	if len(e.Notes) > 0 {
		p.Add(hump.BigHumpName(e.Name), e.Type, tag, "// "+e.Notes)
	} else {
		p.Add(hump.BigHumpName(e.Name), e.Type, tag)
	}
...
func (s *GenStruct) GenerateTableName() []string {
	tmpl, err := template.New("gen_tnf").Parse(GetGenTableNameTpl())
	if err != nil {
		panic(err)
	}
	var data struct {
...
func (s *GenStruct) GenerateColumnName() []string {
	tmpl, err := template.New("gen_tnc").Parse(GetGenColumnNameTpl())
	if err != nil {
		panic(err)
	}
	var data struct {
